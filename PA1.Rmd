...
mean_steps_per_interval_day_types <- ddply(data_no_nas_with_factors, .(interval), summarise, mean_steps_per_interval = mean(steps, na.rm=TRUE))
with(mean_steps_per_interval, plot(interval, mean_steps_per_interval_day_types, type='l', main = "Mean steps per interval averaged over each day"))
with(mean_steps_per_interval_day_types, xyplot(steps ~ interval | day_type, layout = c(1,2), type="l"))
